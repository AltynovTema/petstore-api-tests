...
def created_user(): # Код внутри этой функции created_user будет выполнен, когда pytest вызовет фикстуру @pytest.fixture, и передаст результат тесту yield username.
    ...
    # Извлекаем имя пользователя из данных, которые мы отправили, так как оно используется как ID для других операций (GET, PUT, DELETE)
    # (Сервер может не возвращать username в ответе на POST /user)
    ...
    requests.delete(f"{BASE_URL}/user/test_user_for_api_test") # Код после yield выполняется ПОСЛЕ теста - очистка
